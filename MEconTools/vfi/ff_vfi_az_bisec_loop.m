...
%%
function [mp_valpol_out, flag] = ff_vfi_az_bisec_loop(varargin)
    ...
    for it_z_ctr = 1:length(ar_z)
        ...
        for it_a_ctr = 1:length(ar_a)
            ...
            if (isnan(fl_opti_saveborr_frac))
                ar_min_max = [0, 1-1E-5];
                ar_val_min_max = zeros(size(ar_min_max));
                for it_minmax = [1,2]
                    [~, ar_val_min_max(it_minmax), ~] = ffi_u_v_ap(...
...
function [fl_dU_dap, fl_aprime] = ...
    ffi_foc_u_v_ap(fl_aprime_frac, ar_a, ...
                   fl_beta, fl_crra, ...
                   fl_resources, it_z_ctr, mt_deri_dev_dap)
...
it_ap_near_lower_idx = sum(ar_a <= fl_aprime);
...
fl_dU_dap = fl_du_dap + fl_beta*fl_deri_dev_dap;

end

% Utility given choices
function [fl_aprime, fl_val, fl_c] = ffi_u_v_ap(...
    fl_aprime_frac, ar_a, ...
    fl_beta, fl_crra, ...
    fl_resources, it_z_ctr, mt_ev_ap_z, mt_deri_dev_dap)
% u is current utility, v is future, ev is integrated future, interpolated
...
fl_ev_aprime_z = fl_ev_ap_lower_idx + (fl_aprime - ar_a(it_ap_near_lower_idx))*fl_deri_dev_dap;
